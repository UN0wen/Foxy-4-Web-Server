...
std::map<std::string, std::map<std::string, int>> DataCollector::status_map;
std::map<std::string, std::string> DataCollector::uri_request_handler;
...
void DataCollector::increment_request(std::string uri, Response::status_code status_code )
{
    ...
    if(status_map.count(uri) == 1){
        if(status_map[uri].count(status) == 1){
            status_map[uri][status]++;
        }
        else{
            status_map[uri][status] = 1;
        }
    }
    else{
         status_map[uri][status] = 1;
