...
RequestHandler *ProxyRequestHandler::Init(const std::string &location_path, const NginxConfig &config)
{
    ...
    BOOST_LOG_TRIVIAL(info) << "[ProxyRequestHandler] Set up with host: " + root + " and port: " + std::to_string(port);
...
std::string ProxyRequestHandler::convert_to_rawbody(Request request, bool isRedirect)
{
    ...
    {
        std::string uri = request.uri_.substr(path_.length() - 1, request.uri_.length());
        if (uri[0] != '/') {
            uri.insert(0, "/");
        }
        req += uri;
...
Response ProxyRequestHandler::handle_request(const Request &client_request)
{
    ...
    try
    {
        ...
        while (loop_count < 5)
        {
            ...
            tcp::resolver::query query(root, std::to_string(port_));
...
std::string ProxyRequestHandler::clean_html(std::string &body)
{
    ...
    while (true)
    {
        ...
        src_i = body.find("src");
        if (href_i == std::string::npos && src_i == std::string::npos)
            break;
        else if (href_i == std::string::npos)
            clean_href = false;
        else if (src_i == std::string::npos)
            clean_href = true;
        else
            clean_href = href_i < src_i;
